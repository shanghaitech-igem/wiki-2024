...
- Learn
  ...
  These experimental results suggest the feasibility of using the Flipcherry fluorescence reporting system to characterize the cleavage activity of V8 protease. This system successfully simulates the process in _E. coli_ that mimics the itching sensation on human skin caused by _Staphylococcus aureus_.

## Cycle 3

- Design

  hrough research, we discovered that adding a signal peptide can help V8 protease fold correctly and reduce its toxicity in vivo. Once V8 protease has folded properly, thermolysin can be used to remove the signal peptide, thereby activating the V8 protease. Based on this, we designed a co-transformation strategy in _E. coli_, introducing plasmids carrying both the Flipcherry reporter system and V8 protease with a signal peptide, along with a plasmid expressing thermolysin. We selected pETDuet-1 and pRSFDuet-1 plasmids, as this pair has been validated to successfully co-express in _E. coli_.
